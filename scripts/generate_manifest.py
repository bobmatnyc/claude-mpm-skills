...
class SkillDiscovery:
    ...
    def __init__(
        ...
        if HAS_TIKTOKEN:
            try:
                ...
                print(
                    f"Warning: Failed to load tiktoken encoding: {e}", file=sys.stderr
                )
    ...
    def find_all_skills(self) -> List[Path]:
        ...
        if self.skills_dir is not None:
            # Custom directory mode: scan the user-specified directory directly
            if self.skills_dir.exists():
                skill_files.extend(self.skills_dir.rglob("SKILL.md"))
            else:
                print(
                    f"Warning: skills_dir does not exist: {self.skills_dir}",
                    file=sys.stderr,
                )
        else:
            # Default mode: scan the three canonical directories
            ...
            # Exclude .claude/skills deployment directory (only in default mode)
            skill_files = [f for f in skill_files if ".claude/skills" not in str(f)]
    ...
    def extract_frontmatter(self, skill_path: Path) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"Warning: Failed to extract frontmatter from {skill_path}: {e}",
                    file=sys.stderr,
                )
            return {}
    ...
    def load_metadata_json(self, skill_dir: Path) -> Dict[str, Any]:
        ...
        if metadata_path.exists():
            try:
                ...
                if self.verbose:
                    print(
                        f"Warning: Failed to parse {metadata_path}: {e}",
                        file=sys.stderr,
                    )
        return {}
    ...
    def calculate_token_counts(self, skill_path: Path) -> Tuple[int, int]:
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"Warning: Failed to calculate tokens for {skill_path}: {e}",
                    file=sys.stderr,
                )
            return 0, 0
    ...
    def get_git_last_modified(self, skill_path: Path) -> str:
        ...
        try:
            ...
            if self.verbose:
                print(
                    f"Warning: Git command failed for {skill_path}: {e}",
                    file=sys.stderr,
                )
    ...
    def get_relative_path(self, skill_path: Path) -> str:
        """Get path relative to repo root, optionally prefixed with source_prefix."""
        if self.source_prefix:
            # Use skills_dir as the base if set, otherwise repo_root
            base = self.skills_dir if self.skills_dir is not None else self.repo_root
            try:
                rel = str(skill_path.relative_to(base))
                return f"{self.source_prefix}/{rel}"
            except ValueError:
                return f"{self.source_prefix}/{skill_path.name}"
        try:
    ...
    def classify_skill(
        self, relative_path: str, metadata_json: Optional[Dict[str, Any]] = None
    ) -> Dict[str, Optional[str]]:
        """Classify skill by category, toolchain, and framework."""
        # Strip source_prefix from relative_path when checking known prefixes
        path_for_classification = relative_path
        if self.source_prefix and relative_path.startswith(self.source_prefix + "/"):
            path_for_classification = relative_path[len(self.source_prefix) + 1 :]

        parts = path_for_classification.split("/")

        if parts[0] == "universal":
            return {"category": "universal", "toolchain": None, "framework": None}
            ...
            return {"category": "example", "toolchain": None, "framework": None}

        # Path doesn't match known prefixes — use skills_dir fallback
        if self.skills_dir is not None:
            # Prefer category from metadata.json if available
            if metadata_json and metadata_json.get("category"):
                return {
                    "category": metadata_json["category"],
                    "toolchain": metadata_json.get("toolchain"),
                    "framework": metadata_json.get("framework"),
                }
            return {"category": "utility", "toolchain": None, "framework": None}

        return {"category": "unknown", "toolchain": None, "framework": None}
...
class ManifestValidator:
    ...
    def validate_path(self, source_path: str) -> bool:
        ...
        # When a source_prefix is configured, accept paths starting with it
        if self.source_prefix:
            if not source_path.startswith(self.source_prefix + "/"):
                self.errors.append(
                    f"Path does not start with configured prefix '{self.source_prefix}/': {source_path}"
                )
                return False
            ...
            # Check existence relative to skills_dir (or repo_root if not set)
            # Strip prefix and look under skills_dir
            rel_within_prefix = source_path[len(self.source_prefix) + 1 :]
            base = self.skills_dir if self.skills_dir is not None else self.repo_root
            full_path = base / rel_within_prefix
            if not full_path.exists():
                self.errors.append(f"Path doesn't exist: {full_path}")
                return False

            return True

        # Default mode: require canonical prefixes
        if not (
            source_path.startswith("universal/")
            or source_path.startswith("toolchains/")
            or source_path.startswith("examples/")
        ):
    ...
    def validate_skill(self, skill: Dict[str, Any]) -> bool:
        ...
        for field in required_fields:
            if field not in skill:
                self.errors.append(
                    f"Missing required field '{field}' in skill: {skill.get('name', 'unknown')}"
                )
                valid = False
        ...
        # Validate category — allow 'utility' and any custom category from metadata.json
        # when a skills_dir is configured; otherwise enforce canonical categories
        allowed_categories = ["universal", "toolchain", "example"]
        if self.skills_dir is not None:
            allowed_categories.append("utility")
            # Also allow any non-empty string (metadata.json may define custom categories)
            if skill.get("category"):
                pass  # Any category is acceptable in custom-dir mode
            else:
                self.errors.append(f"Missing category for skill: {skill.get('name')}")
                valid = False
        elif skill.get("category") not in allowed_categories:
            self.errors.append(
                f"Invalid category '{skill.get('category')}' for skill: {skill.get('name')}"
            )
            valid = False
        ...
        if entry_tokens < 10 or entry_tokens > 200:
            self.warnings.append(
                f"Unusual entry_point_tokens ({entry_tokens}) for skill: {skill.get('name')}"
            )

        if full_tokens < 100 or full_tokens > 50000:
            self.warnings.append(
                f"Unusual full_tokens ({full_tokens}) for skill: {skill.get('name')}"
            )

        if entry_tokens > full_tokens:
            self.errors.append(
                f"entry_point_tokens > full_tokens for skill: {skill.get('name')}"
            )
            valid = False
        ...
        if not re.match(r"^\d+\.\d+\.\d+$", version):
            self.warnings.append(
                f"Invalid version format '{version}' for skill: {skill.get('name')}"
            )
        ...
        if not re.match(r"^\d{4}-\d{2}-\d{2}$", updated):
            self.errors.append(
                f"Invalid date format '{updated}' for skill: {skill.get('name')}"
            )
            valid = False
    ...
    def check_duplicates(self, skills: List[Dict[str, Any]]) -> bool:
        ...
        if duplicates:
            ...
            self.errors.append(
                f"Duplicate skill names found: {', '.join(unique_duplicates)}"
            )
            return False
...
class ManifestGenerator:
    ...
    def __init__(
        ...
        self.discovery = SkillDiscovery(
            repo_root, verbose, skills_dir=skills_dir, source_prefix=source_prefix
        )
    ...
    def generate_manifest(self) -> Dict[str, Any]:
        ...
        for i, skill_path in enumerate(skill_files, 1):
            if self.verbose:
                # Use skills_dir as base when set (file may be outside repo_root)
                base = (
                    self.skills_dir if self.skills_dir is not None else self.repo_root
                )
                try:
                    display_path = skill_path.relative_to(base)
                except ValueError:
                    display_path = skill_path
                print(f"Processing {i}/{len(skill_files)}: {display_path}")
        ...
        if example_skills:
            manifest["skills"]["examples"] = sorted(
                example_skills, key=lambda s: s["name"]
            )
...
def main():
    ...
    parser.add_argument(
        "--dry-run", action="store_true", help="Preview changes without writing file"
    )
    ...
    parser.add_argument(
        ...
        type=Path,
        default=None,
        help="Custom directory to scan for skills (default: scans universal/, toolchains/, examples/ at repo root)",
    )
    ...
    parser.add_argument(
        ...
        type=str,
        default="",
        help="Prefix to prepend to source_path entries in manifest (e.g. .claude/skills)",
    )
    ...
    # Bypass the repo_root validity guard when --skills-dir is explicitly provided
    if args.skills_dir is None:
        if (
            not (repo_root / "universal").exists()
            or not (repo_root / "toolchains").exists()
        ):
